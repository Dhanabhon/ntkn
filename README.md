...
### Scope of Token Counting

> [!NOTE]
> `ntkn` **does not count the actual tokens billed by the LLM providers' APIs (API Billing Tokens)**.
> Since `ntkn` operates as an external project observer, it has no visibility into your live chat history or direct prompts sent to the AI Agent.

*   **What `ntkn` counts (Project Context Size):** The total token size of the code and assets in your project folder that are sent as the context payload for the AI Agent to read. This payload typically constitutes the **vast majority of input token costs** when using AI agents.
*   **What is excluded:** Your direct user prompts, historical chat history, and the responses generated by the AI (output tokens), which are all processed dynamically on the provider's API.
